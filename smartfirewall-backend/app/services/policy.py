...
def pending_should_block(approved: int, manual_block: int = 0) -> bool:
    """
    Pending devices should only be forcibly isolated when one of these is true:
    - auto_block_unknown_devices runtime setting is enabled, or
    - the device already has an explicit stored internet_blocked flag.

    This keeps the behavior aligned with the configurable runtime setting and
    avoids treating every pending device as auto-blocked unconditionally.
    """
    if int(approved) == 1:
        return False

    if is_setting_enabled("auto_block_unknown_devices", False):
        return True

    return int(manual_block) == 1
    ...
    for mac, grp in winner.items():
        ...
        if st["approved"] == 0:
            enforce_pending_block(
                mac,
                should_block=pending_should_block(st["approved"], st["manual_block"]),
            )
            continue
        else:
            enforce_pending_block(mac, should_block=False)
